      ...
      {isOpen && (
        <motion.div
          ...
          className={cn(
            ...
            "bg-surface-primary border border-border-subtle rounded-[22px] shadow-ag-modal overflow-hidden"
          )}
        >
          {/* Header */}
            <div className="flex items-center justify-between p-5 border-b border-border-subtle bg-surface-shell shrink-0">
            ...
            {/* Chat Area */}
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 hide-scrollbar bg-surface-primary">
              {messages.map((msg, i) => (
            ...
            <div className="p-3 border-t border-border-subtle bg-surface-elevated shrink-0">
              <form onSubmit={handleSubmit} className="relative">
                ...
                <button 
                  type="submit" 
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg bg-surface-card text-text-secondary hover:text-brand-emerald hover:bg-brand-emerald/10 flex items-center justify-center transition-colors"
                >
                  <ArrowRight size={14} />
                </button>
              </form>
