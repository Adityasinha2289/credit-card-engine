...
// api/admin/entities.ts
var entities_default = requireAdmin(async (req, res) => {
  if (!supabaseAdmin) {
  ...
  if (req.method === "GET") {
    if (id) {
      const { data: data2, error: error2 } = await supabaseAdmin.from("commerce_entities").select(`*, partners(name), categories(name)`).eq("id", id).single();
      if (error2) return res.status(404).json({ success: false, error: "Entity not found" });
      return res.status(200).json({ success: true, data: data2 });
    }
    const { data, error } = await supabaseAdmin.from("commerce_entities").select(`
        *,
  ...
  if (req.method === "POST") {
    ...
    if (!partner_id || !name || !entity_type || base_price === void 0 || !destination_path) {
    ...
    const { data, error } = await supabaseAdmin.from("commerce_entities").insert(req.body).select().single();
    if (error) return res.status(500).json({ success: false, error: error.message });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await supabaseAdmin.from("commerce_entities").update(updates).eq("id", id).select().single();
    if (error) return res.status(500).json({ success: false, error: error.message });
    return res.status(200).json({ success: true, data });
  }
