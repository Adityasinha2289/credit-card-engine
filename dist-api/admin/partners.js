...
// api/admin/partners.ts
var partners_default = requireAdmin(async (req, res) => {
  if (!supabaseAdmin) {
  ...
  if (req.method === "GET") {
    if (id) {
      const { data: data2, error: error2 } = await supabaseAdmin.from("partners").select("*").eq("id", id).single();
      if (error2) return res.status(404).json({ success: false, error: "Partner not found" });
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await supabaseAdmin.from("partners").insert({ name, slug, description, logo_url, is_sponsored, status, primary_category_id }).select().single();
    if (error) {
      if (error.code === "23505") {
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await supabaseAdmin.from("partners").update(updates).eq("id", id).select().single();
    if (error) {
      if (error.code === "23505") {
        return res.status(409).json({ success: false, error: "Partner slug already exists" });
      }
