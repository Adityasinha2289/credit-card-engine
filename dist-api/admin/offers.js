...
async function verifyAdminAuthorization(req) {
  ...
  try {
    const verifiedClaims = await verifyToken2(token, {
      secretKey
    });
...
// api/admin/offers.ts
var offers_default = requireAdmin(async (req, res) => {
  if (!supabaseAdmin) {
  ...
  if (req.method === "GET") {
    if (id) {
      const { data: data2, error: error2 } = await supabaseAdmin.from("offers").select("*").eq("id", id).single();
      if (error2) return res.status(404).json({ success: false, error: "Offer not found" });
  ...
  if (req.method === "POST") {
    ...
    if (!source || !offer_type || value === void 0 || !title || !valid_from || !valid_until) {
    ...
    if (min_spend !== void 0 && min_spend < 0) return res.status(400).json({ success: false, error: "min_spend cannot be negative" });
    if (max_discount !== void 0 && max_discount < 0) return res.status(400).json({ success: false, error: "max_discount cannot be negative" });
    ...
    if (min_spend !== void 0 && min_spend > 0) safeEligibility.minSpend = min_spend;
    if (max_discount !== void 0 && max_discount > 0) safeEligibility.maxDiscount = max_discount;
    const payload = {
      source,
      offer_type,
      value,
    ...
    const { data, error } = await supabaseAdmin.from("offers").insert(payload).select().single();
    if (error) return res.status(500).json({ success: false, error: error.message });
  ...
  if (req.method === "PATCH") {
    ...
    if (updates.value !== void 0 && updates.value < 0) return res.status(400).json({ success: false, error: "Value cannot be negative" });
    ...
    if (updates.min_spend !== void 0 && updates.min_spend < 0) return res.status(400).json({ success: false, error: "min_spend cannot be negative" });
    if (updates.max_discount !== void 0 && updates.max_discount < 0) return res.status(400).json({ success: false, error: "max_discount cannot be negative" });
    ...
    if (updates.eligibility_rules) {
      ...
      if (updates.min_spend !== void 0 && updates.min_spend > 0) safeEligibility.minSpend = updates.min_spend;
      if (updates.max_discount !== void 0 && updates.max_discount > 0) safeEligibility.maxDiscount = updates.max_discount;
