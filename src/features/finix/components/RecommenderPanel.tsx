    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-text-muted mt-0.5">{card.bank} · {card.network}</p>
        </div>
        {/* Match % */}
        <div className="flex-shrink-0 text-right">
          <p className="text-base font-bold text-brand-emerald">{card.matchPercent}%</p>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="text-center p-2 bg-surface-primary  rounded-xl">
          <p className="text-[10px] text-text-muted uppercase tracking-wider">Annual Fee</p>
          <p className="text-xs font-bold text-text-primary mt-0.5">
            {card.annualFee === 0 ? 'Lifetime Free' : `₹${card.annualFee}`}
          </p>
        </div>
        <div className="text-center p-2 bg-surface-primary  rounded-xl">
          <p className="text-[10px] text-text-muted uppercase tracking-wider">Welcome Bonus</p>
          <p className="text-xs font-bold text-text-primary mt-0.5 truncate" title={card.welcomeBonus || 'None'}>
            {card.welcomeBonus || 'None'}
          </p>
        </div>
        <div className="text-center p-2 bg-surface-primary  rounded-xl">
          <p className="text-[10px] text-text-muted uppercase tracking-wider">Lounge Access</p>
          <p className="text-xs font-bold text-text-primary mt-0.5">
      ...
      <button
        ...
        className="flex items-center justify-between text-xs font-bold text-text-secondary hover:text-text-primary transition-colors bg-surface-primary /30 px-3.5 py-2.5 rounded-xl border border-border-subtle dark:border-white/[0.03] mt-1"
      >
      ...
      <AnimatePresence initial={false}>
        {showWhy && (
          <motion.div
            ...
            className="overflow-hidden bg-brand-emerald/[0.01] border border-brand-emerald/10 dark:border-brand-emerald/5 rounded-xl p-3.5 mt-0.5 flex flex-col gap-2.5 text-xs leading-relaxed text-text-secondary"
          >
            {/* Eligibility checks */}
            <div className="flex flex-col gap-1.5 pb-2 border-b border-border-subtle dark:border-white/[0.03]">
              <p className="font-semibold text-text-primary">Profile Requirements Match:</p>
            ...
            {/* Spend Reward Fit */}
            <div>
              <p className="font-semibold text-text-primary mb-1">Spend Category Benefits:</p>
            ...
            {card.highlights && card.highlights.length > 0 && (
              <div className="pt-2 border-t border-border-subtle dark:border-white/[0.03]">
                <p className="font-semibold text-text-primary mb-1">Key Highlights:</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {card.highlights.map((h, idx) => (
                    <span key={idx} className="bg-surface-secondary/60 dark:bg-white/[0.03] text-text-muted px-2 py-0.5 rounded text-[10px] font-medium">
                      {h}
                    </span>
                  ))}
                </div>
      ...
      {getApplyUrl(card.bank) && (
        <a
          ...
          className="mt-1 w-full btn-primary py-2.5 flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(4,59,39,0.3)] active:scale-[0.98]"
        >
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            ...
            <div className="p-4 bg-brand-emerald/5 border border-brand-emerald/10 rounded-2xl flex items-center justify-between gap-3">
              <div>
              ...
              <button 
                ...
                type="button"
                className="text-xs font-bold text-brand-emerald hover:text-brand-650 bg-brand-emerald-muted px-3.5 py-1.5 rounded-full transition-all flex items-center gap-1 flex-shrink-0"
              >
            ...
            <div>
              <h3 className="text-xl font-display font-bold text-text-primary">Where do you spend most?</h3>
              <p className="text-sm text-text-muted mt-1">Pick up to 4 categories (priority order matters).</p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? 'bg-brand-emerald-muted border-brand-emerald text-brand-600 dark:text-brand-400 shadow-sm ring-1 ring-brand-emerald/20'
                        : 'bg-surface/85 dark:bg-surface/40 hover:bg-surface-secondary dark:hover:bg-white/[0.02] border-border-subtle  text-text-secondary'
                    )}
                  >
                    <IconComponent
                      size={18}
                      className={cn(
                        ...
                        isSelected ? 'text-brand-emerald' : 'text-text-muted'
                      )}
                    />
                    <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-col gap-3 pt-2 border-t border-border-subtle">
              <label className="flex items-center justify-between cursor-pointer">
                <div>
                  <p className="text-sm font-semibold text-text-primary">I want lounge access</p>
                  <p className="text-xs text-text-muted">Airport lounge visits</p>
                </div>
                <button
                  onClick={() => setWantsLounge((v) => !v)}
                  className={cn(
                    'w-11 h-6 rounded-full transition-all duration-200 relative flex-shrink-0',
                    wantsLounge ? 'bg-brand-emerald' : 'bg-surface-elevated'
                  )}
                >
                  <motion.div
