...
if (!supabaseUrl || !supabaseServiceKey) {
  console.warn(
    "[Supabase Admin] Missing URL or Service Role Key. Admin mutations will fail if they require server privileges."
  );
}
var supabaseAdmin = supabaseUrl && supabaseServiceKey ? createClient(supabaseUrl, supabaseServiceKey, {
  auth: {
...
// src/features/commerce/services/CommissionCalculator.ts
var CommissionCalculator = class {
  /**
   ...
   */
  static calculateExpectedCommission(orderValue, model, terms) {
    if (orderValue < 0) {
      return 0;
    }
    let commission = 0;
...
function verifyWebhookSignature(signature, rawBody) {
  ...
  const expectedSignature = import_crypto.default.createHmac("sha256", expectedSecret).update(rawBody).digest("hex");
  try {
...
async function handler(req, res) {
  ...
  if (!payload.click_id || !payload.transaction_id || payload.order_value === void 0) {
  ...
  try {
    const { data: trackingEvent, error: trackingError } = await supabaseAdmin.from("tracking_events").select("id, user_id, partner_id, partners(slug)").eq("id", payload.click_id).single();
    if (trackingError || !trackingEvent) {
      return res.status(404).json({ success: false, error: "Tracking event not found" });
    }
    const partnerSlug = trackingEvent.partners?.slug;
    ...
    const { data: existingConversion, error: existingErr } = await supabaseAdmin.from("conversions").select("id, status, partner_id").eq("external_transaction_id", payload.transaction_id).maybeSingle();
    ...
    if (existingConversion) {
      ...
      if (existingConversion.status !== payload.status) {
        await supabaseAdmin.from("conversions").update({ status: payload.status, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", existingConversion.id);
        if (payload.status === "rejected") {
          await supabaseAdmin.from("commissions").update({ status: "voided", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("conversion_id", existingConversion.id);
        }
      }
      return res.status(200).json({ success: true, message: "Conversion updated idempotently" });
    }
    const { data: newConversion, error: conversionError } = await supabaseAdmin.from("conversions").insert({
      ...
      converted_at: payload.occurred_at || (/* @__PURE__ */ new Date()).toISOString()
    }).select("id").single();
    if (conversionError) {
    ...
    const { data: affiliateRel } = await supabaseAdmin.from("affiliate_relationships").select("commission_model, commission_terms").eq("partner_id", trackingEvent.partner_id).eq("status", "active").single();
    if (affiliateRel) {
