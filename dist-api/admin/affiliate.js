...
// api/admin/affiliate.ts
var affiliate_default = requireAdmin(async (req, res) => {
  if (!supabaseAdmin) {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await supabaseAdmin.from("affiliate_relationships").insert(req.body).select().single();
    if (error) return res.status(500).json({ success: false, error: error.message });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await supabaseAdmin.from("affiliate_relationships").update(updates).eq("id", id).select().single();
    if (error) return res.status(500).json({ success: false, error: error.message });
    return res.status(200).json({ success: true, data });
  }
